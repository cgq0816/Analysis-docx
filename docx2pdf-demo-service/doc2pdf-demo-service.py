...
# business logical
from models.docx_to_pdf import Doc2pdf
import traceback
...
class Handler1(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        try:
            ...
            # service 获取参数
            doc_url = post_data['doc_url']
            logging.info('doc_url: {}'.format(doc_url))

            # 模型调用
            result = self.application.model.analysis(doc_url)
            logging.info('result: {}'.format(result))

            # result（模型输出） 可以是任何结构，如 String、List、Dict
            Restful_Result = {"url": result,
                              "message": {"code": 0,
                                          "message": "success"}}
